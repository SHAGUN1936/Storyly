...
export default function Admin() {
  ...
  const addBlockQuick = (kind) => {
    ...
    if (kind === 'music') {
      ...
      block = { type: 'subheading', text: `≡ƒÄ╡ ${track}` };
    }
    if (kind === 'button') {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold gradient-text">Storyly ΓÇö Admin</h1>
          <p className="text-zinc-400 text-sm mt-1 max-w-xl">
            Build websites in the visual editor ΓÇö pages, photos, colors, and floating or stable buttons. No HTML required.
          </p>
        </div>
        <motion.button
            ...
            >
              <div className="flex items-center gap-4">
                {t.thumbnailUrl ? (
                  ...
                  <div className="w-20 h-14 rounded-lg bg-white/5 flex items-center justify-center text-2xl">≡ƒÄ¼</div>
                )}
                <div>
                  <h3 className="font-semibold text-white">{t.name}</h3>
                  <p className="text-sm text-zinc-500">
                    {t.category} ΓÇó {t.published ? 'Published' : 'Draft'}
                  </p>
                </div>
              </div>
          ...
          >
            ...
            <p className="text-sm text-zinc-500 mb-6 max-w-2xl">
              {editing || createStep === 2
                ? 'Edit like an Instagram story: the phone below is the real page ΓÇö add blocks, timers, and buttons. No separate preview column.'
                : 'First name your design, then youΓÇÖll build each page in the story editor.'}
            </p>
              ...
              <div className="space-y-5">
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <p className="text-xs text-zinc-500 mb-2">Optional ΓÇö JPG, PNG, or WebP.</p>
                    <input
                      type="file"
                    ...
                    <div className="space-y-3 border-t border-white/5 pt-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`mt-1 w-full ${APP_INPUT_CLASS}`}
                          placeholder="Something special is comingΓÇª"
                        />
                      </div>
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder="≡ƒÄë Happy Birthday! ≡ƒÄé"
                        />
                      </div>
                    </div>
                  )}
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            try {
                              ...
                              const page = normalizedBuilder.pages[activePage];
                              if (!page) return;
                              const blocks = [...(page.blocks || []), { type: 'subheading', text: `≡ƒÄ╡ ${f.name}`, songUrl: url }];
                              setPageAt(activePage, { ...page, blocks });
                              setSelectedBlockIndex(blocks.length - 1);
                            } catch (err) {
            ...
            <div className="flex gap-3 mt-8 flex-wrap">
              ...
              >
                {formSubmitting ? 'SavingΓÇª' : editing ? 'Save changes' : 'Create website'}
              </button>
            </div>
            )}
          </motion.form>
